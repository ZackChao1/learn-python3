#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 10/17/2018 3:19 PM
# @Author  : Aries
# @Site    : 
# @File    : ushuffle_sad.py
# @Software: PyCharm
...
Base=declarative_base()     # Active Record 声明层
...
class SQLalchemyTest(object):
    def __init__(self):
        ...
        except ImportError:     # 不支持SQLAlchemy或者没有安装适配器
            raise RuntimeError()
        try:
            eng.connect()
        except exc.OperationalError:    # 连接不可达或者库不存在
            eng=create_engine(dirname(dsn))
            eng.execute('craete database %s' % DBNAME).close()
            eng=create_engine(dsn)

        Session=orm.sessionmaker(bind=eng)      # 单独管理事务对象
        self.ses=Session()
